...
public final class CalendarViewContent {
  ...
  private lazy var defaultMonthHeaderItemProvider: (Month) -> AnyCalendarItemModel = { [
    ...
    let itemModel = MonthHeaderView.calendarItemModel(
      ...
      content: .init(monthText: monthText, accessibilityLabel: monthText, monthsLayout: self.monthsLayout))
    return itemModel
  }
  ...
  private lazy var defaultDayOfWeekItemProvider: (Month?, Int)
    -> AnyCalendarItemModel = { [weak self] _, weekdayIndex in
      ...
      let itemModel = DayOfWeekView.calendarItemModel(
        invariantViewProperties: .base,
        content: .init(dayOfWeekText: dayOfWeekText ?? "", accessibilityLabel: dayOfWeekText, monthsLayout: self?.monthsLayout ?? .horizontal))
      return itemModel
    }
